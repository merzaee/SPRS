	...
	<div class="card card-outline card-success">
		...
		<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT users.id, concat(firstname,' ',lastname) as name, users.email, users.password, users.avatar, users.date_created, users.status, users.user_type, users.knowledge_areas, COUNT(paper.p_id) as total_shared FROM users LEFT JOIN paper ON users.id = paper.u_id WHERE users.user_type = 3 GROUP BY users.id;");
					...
					<tr>
						...
						<td><b><?php echo $row['total_reviewed']. ' papers' ?></b></td>
